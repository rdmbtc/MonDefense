...
class SpecializationUI {
  ...
  createUI() {
    ...
    const panelElements = this.uiStyle.createModernPanel(
    ...
    this.title = this.uiStyle.createStyledText(
      ...
      'h1',
      { 
        color: this.uiStyle.colors?.text?.primary || '#ffffff',
        glow: this.uiStyle.colors?.primary?.main || '#4de6ff'
      }
    ).setOrigin(0.5);
  ...
  showPathSelection(specInfo) {
    ...
    paths.forEach(([pathKey, pathData], index) => {
      ...
      const cardElements = this.uiStyle.createModernPanel(x - 125, y - 150, 250, 300);
  ...
  showUpgradePanel(specInfo) {
    ...
    const pathName = this.uiStyle.createStyledText(centerX - 100, centerY - 150, currentPath.name, 'h2', {
      color: this.uiStyle.colors?.primary?.main || '#4de6ff',
      glow: this.uiStyle.colors?.primary?.light || '#66e6ff'
    }).setOrigin(0, 0.5);
    ...
    abilities.forEach(([abilityKey, abilityData], index) => {
      ...
      const abilityElements = this.uiStyle.createModernPanel(centerX - 350, y - 30, 700, 60);
      ...
      this.container.add([abilityElements.panel, abilityElements.glow, abilityName, levelText, progressBar.container, upgradeBtn.container]);
    });
