...
export function ConnectionManager({ 
  ...
  const users = connectedUsers.map(userId => ({
    userId,
    nickname: `User${String(userId).slice(-4)}`,
    isOnline: true
  }))
  const onlineCount = connectedUsers.length

  const connect = async (sessionName?: string, password?: string) => {
    // Mock connection logic
    console.log('Connecting to session:', sessionName, 'with password:', password);
  }
  const disconnect = () => {}
