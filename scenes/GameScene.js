...
// Define the GameScene that extends Phaser.Scene
class GameScene extends Phaser.Scene {
  constructor() {
    super('GameScene');
        ...
        preload() {
          try {
            console.log("GameScene preload started - assets already loaded by LoadingScene");
            
            // Initialize the sound manager (assets already loaded by LoadingScene)
            ...
            this.load.image('chog_attack', '/characters/chog_idle.png'); // Use idle for attack for now
            this.load.image('molandak_idle', '/characters/molandak_idle.png');
            this.load.image('molandak_attack', '/characters/molandak_idle.png'); // Use idle for attack for now
            this.load.image('moyaki_idle', '/characters/moyaki_idle.png');
            this.load.image('moyaki_attack', '/characters/moyaki_idle.png'); // Use idle for attack for now
            this.load.image('keon_idle', '/characters/keon_idle.png');
            this.load.image('keon_attack', '/characters/keon_idle.png'); // Use idle for attack for now
        ...
        createUI() {
          try {
            // Full HD UI scaling
            const uiScale = 2; // Scale UI elements for Full HD
            const uiFontSize = Math.floor(18 * uiScale) + 'px';
            const uiSpacing = 30 * uiScale;
            
            // Create text displays with Full HD scaling
            ...
            this.farmCoinsText = this.add.text(20, 20 + uiSpacing, "Farm Coins: 0", {
              fontFamily: 'Arial',
              fontSize: uiFontSize,
              color: '#FFFF00',
              stroke: '#000000',
            ...
            this.farmCoinsTargetPos = { x: this.farmCoinsText.x + 140, y: this.farmCoinsText.y + 20 };
            
            this.waveText = this.add.text(20, 20 + uiSpacing * 2, "Wave: 0", {
              fontFamily: 'Arial',
              fontSize: uiFontSize,
              color: '#FFFFFF',
              stroke: '#000000',
            ...
            this.livesText = this.add.text(20, 20 + uiSpacing * 3, "Lives: 3", {
              fontFamily: 'Arial',
              fontSize: uiFontSize,
              color: '#FF0000',
              stroke: '#000000',
            ...
            // Add Next Wave button with Full HD scaling
            const buttonScale = 2;
            const nextWaveButton = this.add.rectangle(1920 - 200, 60, 240, 80, 0x00AA00);
            nextWaveButton.setInteractive({ useHandCursor: true });
            nextWaveButton.on('pointerdown', () => {
              this.forceNextWave();
            });
            
            const nextWaveText = this.add.text(1920 - 200, 60, "Next Wave", {
              fontFamily: 'Arial',
              fontSize: Math.floor(16 * buttonScale) + 'px',
              color: '#FFFFFF',
              stroke: '#000000',
              strokeThickness: 2
            }).setOrigin(0.5);
